...
public:
    ...
    enum
    {
        DataHeaderSize = sizeof(Data) - sizeof(T)
    };
...
{
    Q_ASSERT(i >= 0 && i < size()); // "MxList<T>::at", "index out of range"
...
{
    Q_ASSERT(i >= 0 && i < size()); // "MxList<T>::operator[]", "index out of range"
    return *p_at(i);
}
...
{
    Q_ASSERT(i >= 0 && i < size()); // index out of range
    return *p_at(i);
}
