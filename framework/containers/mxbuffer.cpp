...
MxBuffer &MxBuffer::operator=(const MxBuffer &other)
{
    Q_ASSERT(this != &other);
    Q_ASSERT(pSize > 0);

    if (other.pSize > pAlloc)
    {
    ...
    Q_ASSERT(pData && other.pData && pSize > 0);
    memcpy(pData, other.pData, pSize);
...
void MxBuffer::append(const char *data, int size)
{
...
char *MxBuffer::reserveAndGet(int size)
{
    reserveForAppend(size);
